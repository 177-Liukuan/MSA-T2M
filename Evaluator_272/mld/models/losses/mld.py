    ...
    def update(self, rs_set):
        ...
        if self.stage in ["vae", "vae_diffusion"]:
            ...
            total += self._update_loss("recons_joints", rs_set['joints_rst'],
                                       rs_set['joints_ref'])
            if self.vae_type in ["mld_dual_vae"]:
                total += self._update_loss("kl_motionbody", rs_set['body_dist_m'], rs_set['body_dist_ref'])
                total += self._update_loss("kl_motionhand", rs_set['hand_dist_m'], rs_set['hand_dist_ref'])
            else:
                total += self._update_loss("kl_motion", rs_set['dist_m'], rs_set['dist_ref'])
            
            if self.cfg.LOSS.Velocity_loss:
                total += self._update_loss("recons_velocity", rs_set['vel_rst'], rs_set['vel_ref'])
...
class MLDLosses_no_joint(Metric):
    """
    MLD Loss
    """
