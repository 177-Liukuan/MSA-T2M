...
class MLD(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        if self.input_format in ['root_rot6d']:
            example_data = np.load(os.path.join('./HumanML3D-1/joints', '000021' + '.npy'))
            ...
            example_data = np.load('./datasets/Motion-X/motion_data/joint/humanml/000021.npy')
            example_data = example_data.reshape(len(example_data), -1, 3)
            example_data = torch.from_numpy(example_data)
    ...
    def t2m_eval_save_motion_token(self, batch):
        ...
        feats_rst_path = os.path.join(f"./visualization/visualization/{self.cfg.model.vae_type}_VAE_motionx_feats_rst_norm_back", name[0] + '.npy')
    ...
    def t2m_eval_smplx_save_motion_token(self, batch):
        ...
        feats_rst = self.renorm2ori(feats_rst)
        motions = self.renorm2ori(motions)
        feats_rst_path = os.path.join(f"./visualization/visualization/test_case/{self.cfg.TRAIN.DATASETS[0]}/{self.cfg.model.vae_type}_VAE_motionx_feats_rst_norm_back", name[0] + '.npy')
