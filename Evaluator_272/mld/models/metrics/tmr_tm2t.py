...
class TMR_TM2TMetrics(Metric):
    ...
    def __init__(self,
        ...
        self.add_state("TMR_gt_Matching_score",
                       default=torch.tensor(0.0),
                       dist_reduce_fx="sum")
        self.Matching_metrics = ["TMR_Matching_score", "TMR_gt_Matching_score"]
    ...
    def compute(self, sanity_flag):
        ...
        for i in range(count_seq // self.R_size):
            ...
            # [bs=32, 32]
            group_texts = torch.nn.functional.normalize(group_texts, dim=1)
            group_motions = torch.nn.functional.normalize(group_motions, dim=1)

            dist_mat = euclidean_distance_matrix(group_texts,
                                                 group_motions).nan_to_num()
            # print(dist_mat[:5])
            self.TMR_Matching_score += dist_mat.trace()
        ...
        for k in range(self.top_k):
            metrics[f"TMR_R_precision_top_{str(k+1)}"] = top_k_mat[k] / R_count
        ...
        for i in range(count_seq // self.R_size):
            ...
            # match score
            self.TMR_gt_Matching_score += dist_mat.trace()
            argsmax = torch.argsort(dist_mat, dim=1)
            top_k_mat += calculate_top_k(argsmax, top_k=self.top_k).sum(axis=0)
        metrics["TMR_gt_Matching_score"] = self.TMR_gt_Matching_score / R_count
        for k in range(self.top_k):
            metrics[f"TMR_gt_R_precision_top_{str(k+1)}"] = top_k_mat[k] / R_count
