...
class TM2TMetrics_R256(Metric):
    ...
    def __init__(self,
        ...
        for k in range(1, top_k + 1):
            self.add_state(
                f"R_precision_top_{str(k)}_{str(self.R_size)}",
                default=torch.tensor(0.0),
                dist_reduce_fx="sum",
            )
            self.Matching_metrics.append(f"R_precision_top_{str(k)}_{str(self.R_size)}")
        for k in range(1, top_k + 1):
            self.add_state(
                f"gt_R_precision_top_{str(k)}_{str(self.R_size)}",
                default=torch.tensor(0.0),
                dist_reduce_fx="sum",
            )
            self.Matching_metrics.append(f"gt_R_precision_top_{str(k)}_{str(self.R_size)}")
    ...
    def compute(self, sanity_flag):
        ...
        for k in range(self.top_k):
            metrics[f"R_precision_top_{str(k+1)}_{self.R_size}"] = top_k_mat[k] / R_count
        ...
        for k in range(self.top_k):
            metrics[f"gt_R_precision_top_{str(k+1)}_{self.R_size}"] = top_k_mat[k] / R_count
