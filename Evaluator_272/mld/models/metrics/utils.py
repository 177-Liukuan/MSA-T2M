...
def align_by_parts_hand(joints, align_inds=None):
    if align_inds is None:
        return joints

    align_inds_left_wrist = align_inds[0]
    align_inds_right_wrist = align_inds[1]
    
    left_wrist = joints[:, align_inds_left_wrist].mean(1)
    right_wrist = joints[:, align_inds_right_wrist].mean(1)
    result = torch.cat((joints[...,22:22+15,:] - torch.unsqueeze(left_wrist, dim=1), joints[...,22+15:,:] - torch.unsqueeze(right_wrist, dim=1)), dim=-2)
    return result
...
def calc_mpjpe_hand(preds, target, align_inds, sample_wise=True, trans=None):
    assert len(align_inds) == 2
    # align_inds_left_wrist = align_inds[0]
    # align_inds_right_wrist = align_inds[1]
    # Expects BxJx3
    valid_mask = target[:, :, 0] != -2.0
    ...
    if align_inds is not None:
        preds_aligned = align_by_parts_hand(preds, align_inds=align_inds)
        if trans is not None:
            preds_aligned += trans
        target_aligned = align_by_parts_hand(target, align_inds=align_inds)
    else:
        preds_aligned, target_aligned = preds, target
    

    mpjpe_each = compute_mpjpe(preds_aligned[...,-30:,:],
                               target_aligned[...,-30:,:],
                               valid_mask=valid_mask[..., -30:],
                               sample_wise=sample_wise)
    return mpjpe_each
