...
class RAGRetriever:
    ...
    def __init__(self, hcls_dir, embed_dim, topk=3, device=torch.device('cuda')):
        ...
        if len(vectors) == 0:
            raise RuntimeError(f'No valid {self.embed_dim}-d h_cls vectors found in: {hcls_dir}')
    ...
    def retrieve(self, text_emb):
        ...
        if k < self.topk:
            pad_h = torch.zeros(text_emb.shape[0], self.topk - k, self.embed_dim, device=text_emb.device, dtype=top_hcls.dtype)
...
def load_empty_text_embedding(path, expected_dim, device):
    empty_text_emb = torch.from_numpy(np.load(path).astype(np.float32)).reshape(-1).to(device)
    if empty_text_emb.numel() != expected_dim:
        raise ValueError(
            f'empty text embedding dim mismatch: got {empty_text_emb.numel()}, expected {expected_dim}, file={path}'
        )
    return empty_text_emb


def build_text_embedding(text, text_embed_dim, device):
    if text_embed_dim == 512:
        import clip
    ...
    if text_embed_dim == 768:
        ...
        t5_model.eval()
        text_np = np.asarray(t5_model.encode([text]), dtype=np.float32).reshape(1, -1)
        text_emb = torch.from_numpy(text_np).float().to(device)
        return text_emb

    raise ValueError(f'Unsupported text embedding dim: {text_embed_dim}. Only 512(CLIP) and 768(T5) are supported.')
...
def main():
    ...
    text_embed_dim = int(rag_model.base_model.transformer.cond_embed.in_features)
    print(f'Detected text embedding dim from checkpoint: {text_embed_dim}')

    text_emb = build_text_embedding(args.text, text_embed_dim, comp_device)
    if text_emb.shape[-1] != text_embed_dim:
        raise ValueError(f'text embedding dim mismatch: got {text_emb.shape[-1]}, expected {text_embed_dim}')

    empty_text_path = resolve_empty_text_path(args, text_embed_dim)
    print(f'Using empty text embedding: {empty_text_path}')
    empty_text_emb = load_empty_text_embedding(empty_text_path, text_embed_dim, comp_device)

    retriever = build_retriever(args, text_embed_dim, comp_device)
