...
OUTPUT_DIR="${PILOT_ROOT}/evaluation/${SLUG}"
STATUS_DIR="${PILOT_ROOT}/status"
STATUS_FILE="${STATUS_DIR}/${SLUG}.evaluation.status"
...
on_exit() {
    local code=$?
    if [[ $code -ne 0 ]]; then
        write_status "evaluation_failed" "runner exited with code ${code}"
    fi
}
trap on_exit EXIT

if [[ ! -s "$CHECKPOINT" ]]; then
    echo "Evaluation checkpoint is missing: ${CHECKPOINT}" >&2
    exit 10
fi
if [[ -e "$OUTPUT_DIR" ]]; then
    echo "Fresh evaluation output already exists: ${OUTPUT_DIR}" >&2
    exit 11
fi

export CUDA_VISIBLE_DEVICES="$EVALUATION_GPU"
write_status "evaluation_running"
cd "$REPO_ROOT"
...
if [[ ! -s "${OUTPUT_DIR}/metrics.json" ]]; then
    echo "Evaluation metrics.json is missing for ${SLUG}" >&2
    exit 12
fi
write_status "evaluation_complete"
echo "MSA-VAE pilot evaluation complete: ${SLUG}"
